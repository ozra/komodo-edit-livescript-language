...
"""LiveScript support for codeintel.
...
lang = "LiveScript"
log = logging.getLogger("codeintel.livescript")
#log.setLevel(logging.DEBUG)


# These keywords are copied from "LiveScript-mainlex.udl" - be sure to keep both
# of them in sync.
keywords = [
    # Keywords
        "cool"
...
# lexers. You must define a Lexer class for lang LiveScript. If Komodo's
# scintilla lexer for LiveScript is UDL-based, then this is simply:
#
#   from codeintel2.udl import UDLLexer
#   class LiveScriptLexer(UDLLexer):
#       lang = lang
#
# Otherwise (the lexer for LiveScript is one of Komodo's existing C++ lexers
...
#   class LiveScriptLexer(Lexer):
#       lang = lang
#       def __init__(self):
#           self._properties = SilverCity.PropertySet()
#           self._lexer = SilverCity.find_lexer_module_by_id(ScintillaConstants.SCLEX_LIVESCRIPT)
#           self._keyword_lists = [
...
# examples of usage.
class LiveScriptLangIntel(LangIntel):
    lang = lang
...
#
class LiveScriptBuffer(UDLBuffer):
...
# LiveScript content and returns a description of the code in that file.
# See "cile_livescript.py" for more details.
#
# The CILE Driver is a class that calls this CILE. If LiveScript is
...
class LiveScriptCILEDriver(UDLCILEDriver):
    lang = lang

    def scan_purelang(self, buf):
        import cile_livescript
        return cile_livescript.scan_buf(buf)
...
def register(mgr):
    ...
    mgr.set_lang_info(
        lang,
        silvercity_lexer=LiveScriptLexer(),
        buf_class=LiveScriptBuffer,
        langintel_class=LiveScriptLangIntel,
        import_handler_class=None,
        cile_driver_class=LiveScriptCILEDriver,
