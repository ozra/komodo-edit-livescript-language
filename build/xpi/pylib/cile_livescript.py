...
"""A Code Intelligence Language Engine for the LiveScript language.
...
Module Usage:
    from cile_livescript import scan
    mtime = os.stat("bar.livescript")[stat.ST_MTIME]
    content = open("bar.livescript", "r").read()
    scan(content, "bar.livescript", mtime=mtime)
"""
...
class LiveScriptCILEError(CILEError):
    pass
...
log = logging.getLogger("cile.livescript")
...
def scan_buf(buf, mtime=None, lang="LiveScript"):
    """Scan the given LiveScriptBuffer return an ElementTree (conforming
    to the CIX schema) giving a summary of its code elements.
    
    @param buf {LiveScriptBuffer} is the LiveScript buffer to scan
    ...
    """
    # Dev Notes:
    # - This stub implementation of the LiveScript CILE return an "empty"
    #   summary for the given content, i.e. CIX content that says "there
    #   are no code elements in this LiveScript content".
    # - Use the following command (in the extension source dir) to
    #   debug/test your scanner:
    #       codeintel scan -p -l LiveScript <example-LiveScript-file>
    ...
    # Dev Note:
    # This is where you process the LiveScript content and add CIX elements
    ...
    #   Use the "codeintel html -b <example-LiveScript-file>" command as
    #   a debugging tool.
    # - "buf.accessor.text" is the whole content of the file. If you have
    #   a separate tokenizer/scanner tool for LiveScript content, you may
    #   want to use it.
